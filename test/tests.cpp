...
/*
   F19FS * fs_format(const char *const fname);
   1   Normal
   2   NULL
   3   Empty string
   F19FS *fs_mount(const char *const fname);
   1   Normal
   2   NULL
   3   Empty string
   int fs_unmount(F19FS *fs);
   1   Normal
   2   NULL
 */

TEST(a_tests, format_mount_unmount) {
    const char *test_fname = "a_tests.F19FS";
    F19FS *fs = NULL;
    // FORMAT 2
...
/*
   int fs_create(F19FS *const fs, const char *const fname, const ftype_t ftype);
...
TEST(b_tests, file_creation_one) {
    ...
    const char *test_fname = "b_tests_normal.F19FS";
...
TEST(b_tests, file_creation_two) {
    ...
    const char *test_fname = "b_tests_full_table.F19FS";
    F19FS *fs = fs_format(test_fname);
    ASSERT_NE(fs, nullptr);
...
/*
   int fs_open(F19FS *fs, const char *path)
...
TEST(c_tests, open_close_file) {
   ...
   "more/bad_req",
   "/folder/withfilethatiswayyyyytoolongwhydoyoumakefilesthataretoobigEXACT!", "/", "/mystery_file"};
   const char *test_fname = "c_tests.F19FS";
   ASSERT_EQ(system("cp b_tests_normal.F19FS c_tests.F19FS"), 0);
...
/*
   int fs_get_dir(const F19FS *const fs, const char *const fname, dir_rec_t *const records)
...
/*
   ssize_t fs_write(F19FS *fs, int fd, const void *src, size_t nbyte);
...
/*
   ...
   const char *test_fname = "d_tests_normal.F19FS";
   F19FS *fs = fs_format(test_fname);
   ASSERT_NE(fs, nullptr);
...
TEST(d_tests, write_file_fill) {
    ...
    vector<const char *> fnames{"/file_a", "/file_b", "/file_c", "/file_d"};
    const char *test_fname = "d_tests_full.F19FS";
    F19FS *fs = fs_format(test_fname);
    ASSERT_NE(fs, nullptr);
    uint8_t large_eight_five_b_seven[1024 * 3];
    memset(large_eight_five_b_seven, 0x88, 888);
...
*/
/* 
   int fs_remove(F19FS *fs, const char *path);
   1. Normal, file at root
...
/*
   ...
   const char *(test_fname[2]) = {"e_tests_a.F19FS", "e_tests_b.F19FS"};
   ASSERT_EQ(system("cp d_tests_full.F19FS e_tests_a.F19FS"), 0);
   ASSERT_EQ(system("cp c_tests.F19FS e_tests_b.F19FS"), 0);
   F19FS *fs = fs_mount(test_fname[1]);
   ASSERT_NE(fs, nullptr);
// FS_REMOVE 10
 ...
 */
/*
   off_t fs_seek(F19FS *fs, int fd, off_t offset, seek_t whence)
...
/*
   TEST(g_tests, seek) {
   vector<const char *> fnames{"/file_a", "/file_b", "/file_c", "/file_d"};
   const char *test_fname = "g_tests.F19FS";
   ASSERT_EQ(system("cp d_tests_full.F19FS g_tests.F19FS"), 0);
   F19FS *fs = fs_mount(test_fname);
   ASSERT_NE(fs, nullptr);
 ...
 */
/*
   ssize_t fs_read(F19FS *fs, int fd, void *dst, size_t nbyte);
...
/*
   int fs_move(F19FS *fs, const char *src, const char *dst);
...
/*
   ...
   "/folder/withfilethatiswayyyyytoolongwhydoyoumakefilesthataretoobigEXACT!", "/", "/mystery_file"};
   const char *test_fname = "g_tests.F19FS";
   ASSERT_EQ(system("cp c_tests.F19FS g_tests.F19FS"), 0);
   F19FS *fs = fs_mount(test_fname);
   ASSERT_NE(fs, nullptr);
...
/*
   ...
   Finish both the tests and function implementation of fs_link, and get 20 bonus points!
